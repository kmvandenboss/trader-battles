...
### Phase B — Neon Postgres behind the repository interface (owner: `data-seed` agent) — ✅ DONE
...
scripts, `getRepositories()` env switch, `.env.example` + `docs/database.md`. QA pass. Live smoke
DONE against the real Neon DB (migrated, seeded idempotently, all routes serving from Postgres).
...
### Phase C — Bridge auth (owner: `data-seed` for schema + `frontend-ui`/`general-purpose` for wiring) — ✅ DONE
...
states, Rule-1 labeling off the auth link. QA pass. Live smoke DONE (headless-browser sign-up →
session identity → sign-out fallback → sign-in, DB rows verified; see STATE.md).
